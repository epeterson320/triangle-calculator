...
describe('<RadioList />', () => {
  ...
  it('input elements have the same name attribute', () => {
    const el = render()
    const name1 = el.find('input').first().prop('name')
  ...
  it('checks the option with "default"', () => {
    expect(render().find('input').first().prop('checked')).toBe(true)
  })

  it('switches to a different option when clicked', () => {
    const el = render()
    el.find('input').at(1).simulate('change', { target: { value: 2 } })
    expect(el.find('input').first().prop('checked')).toBe(false)
    expect(el.find('input').at(1).prop('checked')).toBe(true)
  })
  ...
  it('Calls onChange with the values when selected', () => {
    ...
    el.find('input').at(0).simulate('change', { target: { value: 1 } })
    expect(onChange).toHaveBeenCalledWith(1)
    el.find('input').at(2).simulate('change', { target: { value: 3 } })
