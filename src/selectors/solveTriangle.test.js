...
describe('Triangle Solution selector', () => {
  ...
  describe('Errors', () => {
    ...
    it('Flags invalid and unknown input', () => {
      expect(solve({ a: 'Hello', A: 'Hi', b: '13' }).errors).toEqual({
        a: NonNumberInput,
        A: NonNumberInput,
      });

      expect(solve({ b: 'Q', B: 'Hi', c: 'zz', C: ':)' }).errors).toEqual({
        b: NonNumberInput,
        B: NonNumberInput,
        c: NonNumberInput,
        C: NonNumberInput,
    ...
    it('Flags sides that are too short (3 sides)', () => {
      const long = '50';
      const short = '7';
      expect(solve({ a: long, b: short, c: short }).errors).toEqual({
        a: SideTooLong,
        b: SideTooShort,
        c: SideTooShort,
      });
      expect(solve({ b: long, a: short, c: short }).errors).toEqual({
        b: SideTooLong,
        a: SideTooShort,
        c: SideTooShort,
      });
      expect(solve({ c: long, b: short, a: short }).errors).toEqual({
        c: SideTooLong,
        b: SideTooShort,
        a: SideTooShort,
      });
    ...
    it('Flags sides that are too short (2 sides 1 angle)', () => {
      ...
      const longer = '10';
      const angle = '1.5708';
      expect(solve({ A: angle, b: longer, a: shorter }).errors).toEqual({
        a: SideTooShort,
        b: SideTooLong,
      });
      expect(solve({ B: angle, a: longer, b: shorter }).errors).toEqual({
        b: SideTooShort,
        a: SideTooLong,
      });
      expect(solve({ A: angle, c: longer, a: shorter }).errors).toEqual({
        a: SideTooShort,
        c: SideTooLong,
      });
      expect(solve({ C: angle, a: longer, c: shorter }).errors).toEqual({
        c: SideTooShort,
        a: SideTooLong,
      });
      expect(solve({ B: angle, c: longer, b: shorter }).errors).toEqual({
        b: SideTooShort,
        c: SideTooLong,
      });
      expect(solve({ C: angle, b: longer, c: shorter }).errors).toEqual({
        c: SideTooShort,
        b: SideTooLong,
      });
    });
    ...
    it('Flags angles that add up to 180 or more', () => {
      const obtuse = '1.5708';
      expect(solve({ A: obtuse, B: obtuse }).errors).toEqual({
        A: AngleSumTooLarge,
        B: AngleSumTooLarge,
      });
      expect(solve({ A: obtuse, C: obtuse }).errors).toEqual({
        A: AngleSumTooLarge,
        C: AngleSumTooLarge,
      });
      expect(solve({ B: obtuse, C: obtuse }).errors).toEqual({
        B: AngleSumTooLarge,
        C: AngleSumTooLarge,
      });
    });
