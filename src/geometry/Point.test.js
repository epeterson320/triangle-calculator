...
describe('RectPoint', () => {
  ...
  it('Throws an error if constructed with non-number parameters', () => {
    expect(() => {
      RectPoint(0, undefined);
    }).toThrow();
    expect(() => {
      RectPoint('0', 4);
    }).toThrow();
    expect(() => {
      RectPoint(5, Infinity);
    }).toThrow();
  });
  ...
  it('Avoids divide by 0 when computing angle', () => {
    const p = RectPoint(0, 0);
    expect(p.angle).toBeGreaterThanOrEqual(ANG_0);
    expect(p.angle).toBeLessThanOrEqual(ANG_180);
  ...
  it('Tests equality correctly', () => {
    ...
    expect(RectPoint(1, 1).equals(PolarPoint(sqrt(2), ANG_45))).toBeTruthy();
  });
});
