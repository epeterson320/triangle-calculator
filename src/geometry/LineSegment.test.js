...
describe('Line', () => {
  ...
  it('Has a PointAngleDistance constructor', () => {
    ...
    expect(new Line.PointAngleDistance(origin, ANG_180, 4.2)).toBeInstanceOf(
      Line,
    );
  });
  ...
  describe('Equality', () => {
    ...
    it('Compares vertical lines correctly', () => {
      ...
      const l2 = new Line.PointAngleDistance(
        PolarPoint(1, -ANG_90),
        ANG_90,
        10,
      );
  ...
  it('Bisects other lines', () => {
    ...
    expect(bisector1.equals(Line.PointAngleDistance(origin, PI / 8, 1))).toBe(
      true,
    );

    const bisector2 = Line.PointAngleDistance(origin, -PI / 8, 1).bisect(
      Line.PointAngleDistance(origin, (3 * PI) / 8, 1),
    );

    expect(bisector2.angle).toBeCloseTo(
      Line.PointAngleDistance(origin, PI / 8, 1).angle,
    );
  });
  ...
  it('Contains points when extended', () => {
    expect(xEqY.contains(RectPoint(100000, 100000), { extend: true })).toBe(
      true,
    );
  });
});
